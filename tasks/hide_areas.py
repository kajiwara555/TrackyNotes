...
def main():
    ...
    # set desk by myself
    desk_area = np.array([[1655, 725], [2300, 1255]])
    ...
    mask_num_list = mask_num_list[141:199]
    previous_mask_len = 0

    # set mask area by manual
    mask_area = [[116, 108], [143, 119], [164, 107], [211, 119], [237, 104], [271, 117], [313, 102], [326, 120],
                 [357, 106], [401, 120], [428, 104], [470, 121], [495, 104], [522, 118], [507, 128], [529, 174],
                 [510, 198], [531, 230], [510, 252], [530, 266], [509, 292], [530, 328], [508, 351], [537, 388],
                 [498, 402], [526, 414], [354, 397], [404, 410], [310, 401], [332, 416], [236, 398], [290, 411],
                 [167, 401], [213, 413], [118, 400], [147, 412], [114, 346], [130, 386], [114, 286], [131, 327],
    ...
    for mask_num in mask_num_list:
        ...
        while (1):
            ...
            for mask_num_each in mask_num:
                cv2.rectangle(frame, (mask_area[mask_num_each * 2][0], mask_area[mask_num_each * 2][1]),
                              (mask_area[mask_num_each * 2 + 1][0], mask_area[mask_num_each * 2 + 1][1]),
                              (200, 200, 200), -1)
            ...
            for i in range(0, len(postit_points)):
                ...
                try:
                    ...
                    #     print each_bit
                    # postit_image_for_save = cv2.imread("./tmp_datas/postit_tmp/" + str(0) + ".jpg")
                    # postit_image_for_save2 = cv2.resize(postit_image_for_save, (
                    #     int(postit_image_for_save.shape[1] * 0.1), int(postit_image_for_save.shape[0] * 0.1)))
                    # cv2.imshow("homographied", postit_image_for_save2)
                    # cv2.waitKey(0)
