...
def main():
    ...
    for mask_num in mask_num_list:
        ...
        frame = cv2.imread('./datas/movie/' + movie_files[movie_id])  # save frame for final
        ...
        for mask_num_each in mask_num:
            cv2.rectangle(frame, (mask_area[mask_num_each * 2][0], mask_area[mask_num_each * 2][1]),
                          (mask_area[mask_num_each * 2 + 1][0], mask_area[mask_num_each * 2 + 1][1]), (200, 200, 200), -1)
        ...
        for i in range(0, len(postit_points)):
            ...
            if result_num != -1:
                ...
                if postit_saved.has_key(result_num):
                    ...
                    # calc dist
                    dist = np.linalg.norm(
                        np.mean(postit_saved[result_num]["points_saved"], axis=0) - np.mean(postit_points[i], axis=0))
                    # calc angle(degree)
                ...
                # first appear
                    ...
                    postit_saved[result_num] = {"points": postit_points[i], "points_saved": postit_points[i],
                                                "first_time": time / fps, "last_time": time / fps, "move": [], "rotate": []}
        ...
        for key in postit_saved:
            ...
            cv2.putText(frame_for_final, str(key), (np.mean(postit_saved[key]["points"], axis=0).astype(np.int)[0] - 40,
                                                    np.mean(postit_saved[key]["points"], axis=0).astype(np.int)[1]),
