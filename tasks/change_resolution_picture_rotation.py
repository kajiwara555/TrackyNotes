...
def main():
    ...
    for rotate_angle in range(90, 105, 15):
        experiment_result.write("---angle:" + str(rotate_angle) + "---" + "\n")
        for movie_id in range(len(movie_files)):
            ...
            for resolution in range(0, 99):
                ...
                scale_down_ratio = float(postit_area_width) / frame.shape[1]
                # frame = cv2.resize(frame, (postit_area_width, postit_area_height), interpolation=cv2.INTER_LANCZOS4)
                # rotate the frame
                rotate_M = np.array(
                    [[np.cos(rotate_angle * np.pi / 180), -np.sin(rotate_angle * np.pi / 180), postit_area_width * 2],
                     [np.sin(rotate_angle * np.pi / 180), np.cos(rotate_angle * np.pi / 180), postit_area_height / 2]])
                frame = cv2.warpAffine(frame, rotate_M, (postit_area_width * 4, postit_area_height * 4))
                ...
                frame = cv2.resize(frame,
                                   (int(frame.shape[1] * scale_down_ratio * (1 - resolution)),
                                    int(frame.shape[0] * scale_down_ratio * (1 - resolution))),
