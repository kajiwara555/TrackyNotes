...
def main():
    ...
    for rotate_angle in range(0, 105, 15):
        ...
        for movie_id in range(len(movie_files)):
            for resolution in range(0, 100):
                ...
                while (1):
                    ...
                    # rotate the frame
                    rotate_M = np.array(
                        [[np.cos(rotate_angle * np.pi / 180), -np.sin(rotate_angle * np.pi / 180),
                          postit_area_width * 2],
                         [np.sin(rotate_angle * np.pi / 180), np.cos(rotate_angle * np.pi / 180),
                          postit_area_height / 2]])
                    frame = cv2.warpAffine(frame, rotate_M, (frame.shape[1] * 4, frame.shape[0] * 4))

                    # change resolution
                    frame = cv2.resize(frame,
                                       (int(frame.shape[1] * (1 - resolution)), int(frame.shape[0] * (1 - resolution))),
