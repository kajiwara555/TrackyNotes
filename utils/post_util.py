...
rect_rect_space_vertical = (
                               postit_height / 2 - horizon_y_buffer - location_height - location_width / 2 - bit_width * 2) / 3
...
point_buffer = read_config[
                   "point_buffer"] * expand_ratio  # used when searching in larger area and extract larger area of location point area
larger_buffer = read_config[
                    "larger_buffer"] * expand_ratio  # used for setting larger area in whole postit area for analyzing
...
find_grand_child = read_config["find_grand_child"]
...
class getPostits:
    ...
    def __init__(self, frame, outer_size):
        ...
        for count in range(0, len(contours)):
            ...
            if area > outer_size * outer_lower and area < outer_size * outer_upper:
                ...
                if idx != -1:
                    ...
                    if idx2 != -1 or (not find_grand_child):
                        cv2.drawContours(frame, [box], 0, (0, 0, 255), 2)
                        cv2.putText(frame, str(area), (box[0][0], box[0][1]), cv2.FONT_HERSHEY_PLAIN, 5.0, (0, 128, 0),
                                    5)
                        ...
                        point_buffer_for_larger = int(
                            (point_buffer / expand_ratio) * ((float(outer_size) / 220) ** 0.5))
                        ...
                        for count_in_larger in range(0, len(contours_in_larger)):
                            ...
                            if area_in_larger > outer_size * outer_lower and area_in_larger < outer_size * outer_upper:
                                ...
                                if idx != -1:
                                    ...
                                    for i in range(0, 4):
                                        # box[i] = np.int0([min_point[0] - point_buffer_for_larger,
                                        #                   min_point[1] - point_buffer_for_larger]) + box_in_larger[i]
                                        box[i] = np.int0([min_x,
                                                          min_y]) + box_in_larger[i]
                                    flag_changed = True

                        # find candidate of left-top
                        candidate = []
                        for i in range(0, 4):
                            if np.linalg.norm(box[i] - box[(i + 1) % 4]) > np.linalg.norm(
                                            box[(i + 1) % 4] - box[(i + 2) % 4]):
                                candidate.append(i)
                        ...
                        for id in candidate:
                            ...
                            after_point = np.float32([[0 + point_buffer, 0 + point_buffer],
                                                      [location_width + point_buffer, 0 + point_buffer],
                                                      [location_width + point_buffer, location_height + point_buffer],
                                                      [0 + point_buffer, location_height + point_buffer]])
                            M = cv2.getPerspectiveTransform(before_point, after_point)
                            dst = cv2.warpPerspective(frame_original, M, (
                                location_width + point_buffer * 2, location_height + point_buffer * 2))
                            ...
                            if flag_changed or (not flag_changed):  # changed20161109
                                ...
                                for count_a in range(0, len(contours_a)):
                                    ...
                                    if idx_a != -1 and area_a > location_area_size * 0.7 and area_a < location_area_size * 1.3:
                                        ...
                                        if box_argsort[0][1] == box_argsort[0][0] or box_argsort[1][1] == \
                                                box_argsort[0][0]:
                            ...
                            location_ratio = (
                                float(area_width + area_height) / (location_width + location_height + line_width * 2))
                            space_x_mod = space_x * location_ratio
                            to_center_dst = (location_width / 2 - (
                                x_buffer + rect_len / 2 + space_x * 2)) * location_ratio

                            dot_point = [0 for i in range(0, location_dot_num)]
                            for i in range(0, location_dot_num):
                                dst_area = dst[int(box_a_sorted[0][1] + area_height / 2 - dot_read_area):int(
                                    box_a_sorted[0][1] + area_height / 2 + dot_read_area),
                                           (int(center_x - to_center_dst + space_x_mod * (i - 2)) - dot_read_area):(
                                               int(center_x - to_center_dst + space_x_mod * (i - 2)) + dot_read_area)]
                                if np.mean(dst_area) < dot_read_thre:
                                    dot_point[i] = 1
                            #         cv2.circle(dst, (int(center_x - to_center_dst + space_x_mod * (i - 2)),
                            #                          int(box_a_sorted[0][1] + area_height / 2)), dot_read_area, 255, 3)
                            #         cv2.circle(dst, (int(center_x - to_center_dst + space_x_mod * (i - 2)),
                            #                          int(box_a_sorted[0][1] + area_height / 2 + 12)), dot_read_area, 0,
                            #                    3)
                            # cv2.rectangle(dst, (box_a_sorted[0][0], box_a_sorted[0][1]),
                            #               (box_a_sorted[2][0], box_a_sorted[2][1]), 0)
                            # cv2.imshow("dst_orig", dst)
                            # cv2.waitKey(0)
                            if dot_point[0] == 1 and dot_point[location_dot_num - 1] == 0:
                                ...
                                before_point = np.float32([[0 + point_buffer, 0 + point_buffer],
                                                           [location_width + point_buffer, 0 + point_buffer],
                                                           [location_width + point_buffer,
                                                            location_height + point_buffer],
                                                           [0 + point_buffer, location_height + point_buffer]])
                                ...
                                for box_a_each in box_3d:
                                    ...
                                    cv2.circle(frame, (int(box_a_after_each[0]), int(box_a_after_each[1])), 5,
                                               (100, 100, 100), 5)

                                location_xy[dot_id].append(np.float32(box_a_after))
                                # write each id in this point
                                cv2.putText(frame, str(dot_id), (np.mean(box_a_after, axis=0).astype(np.int)[0],
                                                                 np.mean(box_a_after, axis=0).astype(np.int)[1]),
                                            cv2.FONT_HERSHEY_PLAIN, 5.0, (0, 0, 0), 5)
        ...
        right_basic_vec = np.array(
            [float(postit_width / 2 - horizon_x_buffer - location_width / 2) / (location_width + correction_width),
             0.0])
        under_basic_vec = np.array([0.0, float(postit_height / 2 - horizon_y_buffer - location_height / 2) / (
            location_height + correction_height)])
        ...
        for i in range(0, 8):
            for j in range(0, 8):
                ...
                if (vec_from_to[i][j] == np.array([0, 0]))[0] == True and (vec_from_to[i][j] == np.array([0, 0]))[
                    1] == True:
        ...
        for m in range(0, len(location_xy)):
            ...
            for n in range(0, len(location_xy[m])):
                if location_xy[m][n][0][0] != -1:
                    ...
                    for i in range(0, 8):
                        if i != m:
                            dst_point = (np.mean(location_xy[m][n], axis=0) + vec_from_to[m][i][0] * right_vector +
                                         vec_from_to[m][i][1] * below_vector).astype(np.int)
                            cv2.circle(frame, (dst_point[0], dst_point[1]), 10, (0, 120, 220), 5)
                            for j in range(0, len(location_xy[i])):
                                if location_xy[i][j][0][0] != -1 and np.linalg.norm(
                                                np.mean(location_xy[i][j], axis=0) - dst_point) < error_thresh * (
                                            float(outer_size) / 220) ** 0.5:
                    ...
                    if self.recognized_location_rectangle < points_count + 1:
        ...
        # extract postit area
        # homography
        dst_points_original = [[horizon_x_buffer + location_width / 2, horizon_y_buffer + location_height / 2],
                               [postit_width / 2, horizon_y_buffer + location_height / 2],
                               [postit_width - horizon_x_buffer - location_width / 2,
                                horizon_y_buffer + location_height / 2],
                               [horizon_x_buffer + location_width / 2,
                                postit_height - horizon_y_buffer - location_height / 2],
                               [postit_width / 2, postit_height - horizon_y_buffer - location_height / 2],
                               [postit_width - horizon_x_buffer - location_width / 2,
                                postit_height - horizon_y_buffer - location_height / 2],
                               [horizon_x_buffer + location_height / 2, postit_height / 2],
                               [postit_width - horizon_x_buffer - location_height / 2, postit_height / 2]]
        ...
        for points in location_points_all:
            ...
            try:
                postit_result = cv2.warpPerspective(frame_original, M, (
                    postit_width + larger_buffer * 2, postit_height + larger_buffer * 2))
            except:
                continue
            ...
            try:
                ret, postit_result_bin = cv2.threshold(postit_result_bin, 0.0, 255.0,
            ...
            postit_tmp = postit_result[larger_buffer: (postit_result.shape[0] - larger_buffer),
                         larger_buffer: (postit_result.shape[1] - larger_buffer)]
            ...
            postit_area = []
            postit_area_before = np.array(
                [(0, 0, 1), (postit_width, 0, 1), (postit_width, postit_height, 1), (0, postit_height, 1)])
            postit_points_each = []
    ...
    def read_bit(self, postit, first_x, first_y, horizon):
        ...
        if horizon == True:
            ...
            for i in range(0, bit_num):
                dst_area = search_area[int(center_y - to_center_dst_y + space_x_mod_y * (i - 4) - dot_read_area):int(
                    center_y - to_center_dst_y + space_x_mod_y * (i - 4) + dot_read_area),
                           (int(center_x - to_center_dst_x + space_x_mod_x * (i - 4) - dot_read_area)):(
                               int(center_x - to_center_dst_x + space_x_mod_x * (i - 4) + dot_read_area))]
        ...
        if horizon == False:
            ...
            dot_point = [0 for i in range(0, bit_num)]
            for i in range(0, bit_num):
                dst_area = search_area[(int(center_y - to_center_dst_y + space_x_mod_y * (i - 4) - dot_read_area)):(
                    int(center_y - to_center_dst_y + space_x_mod_y * (i - 4) + dot_read_area)),
                           int(center_x - to_center_dst_x + space_x_mod_x * (i - 4) - dot_read_area):int(
                               center_x - to_center_dst_x + space_x_mod_x * (i - 4) + dot_read_area)]
                if dst_area.shape[0] == 0: break
                if np.mean(dst_area) < dot_read_thre:
                    dot_point[i] = 1
            #     cv2.circle(search_area, (int(center_x - to_center_dst_x + space_x_mod_x * (i-4)),int(center_y -  to_center_dst_y + space_x_mod_y * (i-4)) ), dot_read_area, 255, 3)
            #     cv2.circle(search_area, (int(center_x - to_center_dst_x + space_x_mod_x * (i-4) + 12),int(center_y -  to_center_dst_y + space_x_mod_y * (i-4)) ), dot_read_area, 0, 3)
            # cv2.imshow("search_area", search_area)
            # cv2.waitKey(0)
